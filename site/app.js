...
const TEAM_LOGO_CODES = {LA:'lar',WAS:'wsh'};
const UI_ICONS = {
  pin:'<path d="M12 21s6-5.1 6-11a6 6 0 1 0-12 0c0 5.9 6 11 6 11Z"/><circle cx="12" cy="10" r="2.3"/>',
  bars:'<path d="M5 20V11M12 20V4M19 20v-7"/>',
  target:'<circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/><path d="m14.5 9.5 5-5M16.5 4.5h3v3"/>',
  trophy:'<path d="M8 4h8v4c0 4-2 6-4 6s-4-2-4-6V4Z"/><path d="M8 6H4v2c0 2 1.5 4 4.5 4M16 6h4v2c0 2-1.5 4-4.5 4M12 14v4M8 20h8"/>',
  board:'<path d="M9 6h11M9 12h11M9 18h11"/><circle cx="4" cy="6" r="1"/><circle cx="4" cy="12" r="1"/><circle cx="4" cy="18" r="1"/>',
  chevron:'<path d="m9 5 7 7-7 7"/>',
  info:'<circle cx="12" cy="12" r="9"/><path d="M12 11v6M12 7h.01"/>'
};
const icon = name => `<svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true">${UI_ICONS[name] || ''}</svg>`;
const logoCode = team => TEAM_LOGO_CODES[team] || String(team || '').toLowerCase();
...
function gameCard(game, index, valueRows = [], market = null) {
  ...
  const displayHomeScore = Number(game.display_home_score || 0);
  const displayAwayScore = Number(game.display_away_score || 0);
  const displayMargin = displayHomeScore - displayAwayScore;
  const displayTotal = displayHomeScore + displayAwayScore;
  const spread = displayMargin > 0 ? `${game.home_team} -${Math.abs(displayMargin)}` : displayMargin < 0 ? `${game.away_team} -${Math.abs(displayMargin)}` : "PICK'EM";
  ...
  const awayName = TEAM_NAMES[game.away_team] || game.away_team;
  const homeName = TEAM_NAMES[game.home_team] || game.home_team;
  const awayLogo = `assets/team-logos/${logoCode(game.away_team)}.png`;
  const homeLogo = `assets/team-logos/${logoCode(game.home_team)}.png`;
  const finalScore = game.result_status === 'FINAL' ? `<div class="final-score"><span>FINAL SCORE</span><strong>${esc(game.away_team)} ${esc(game.actual_away_score)} <i>—</i> ${esc(game.actual_home_score)} ${esc(game.home_team)}</strong></div>` : '';
  const marketPanel = finalScore + (market ? `<div class="market-strip"><div class="market-title"><span>MARKET-IMPLIED SCORE ${icon('info')}</span><small>Consensus main spread + total</small></div><div class="market-score"><b>${esc(game.away_team)} ${num(market.market_away_points,0)}</b><i>—</i><b>${num(market.market_home_points,0)} ${esc(game.home_team)}</b></div><div class="market-lines"><span>${esc(market.market_spread_label)}</span><span>O/U ${num(market.market_total)}</span><span>${Math.min(market.spread_book_count,market.total_book_count)}+ books</span></div></div>` : `<div class="market-strip unavailable"><span>Market-implied score unavailable</span></div>`);
  ...
  return `<article class="game-card"><div class="game-head"><div class="game-meta"><b>WEEK ${esc(game.week)}</b><i></i><span>${when(game.kickoff_utc)}</span><i></i><span class="venue">${icon('pin')}${esc(game.stadium || 'Venue TBC')}</span></div><span class="season-chip">NFL 2026</span></div><div class="matchup scoreboard"><div class="matchup-line"><div class="team-logo team-logo-away"><span>${esc(game.away_team)}</span><img src="${awayLogo}" alt="${esc(awayName)} logo" loading="lazy" onerror="this.remove()"></div><div class="score-team score-team-away"><strong>${esc(game.display_away_score)}</strong><span>${esc(awayName)}</span><small>(${esc(game.away_team)})</small></div><div class="fixture-at"><span>@</span></div><div class="score-team score-team-home"><strong>${esc(game.display_home_score)}</strong><span>${esc(homeName)}</span><small>(${esc(game.home_team)})</small></div><div class="team-logo team-logo-home"><span>${esc(game.home_team)}</span><img src="${homeLogo}" alt="${esc(homeName)} logo" loading="lazy" onerror="this.remove()"></div></div></div><div class="quick-stats"><div><span>${icon('bars')}MODEL LINE</span><strong>${esc(spread)}</strong></div><div><span>${icon('target')}MODEL O/U</span><strong>${displayTotal}</strong></div><div><span>${icon('trophy')}${esc(winner)} WIN CHANCE</span><strong>${pct(winProbability)}</strong></div></div>${marketPanel}<details><summary><span class="summary-label">${icon('board')}Open matchup board</span>${icon('chevron')}</summary><div class="details-body"><div class="tabbar" role="tablist"><button class="active" data-tab="overview-${index}">Overview</button><button data-tab="td-${index}">TD scorers</button><button data-tab="rush-${index}">Rushing</button><button data-tab="receive-${index}">Receiving</button><button data-tab="value-${index}">Value</button><button data-tab="insight-${index}">Insight</button></div><div id="overview-${index}" class="tab-panel active"><p><b>Precise model mean:</b> ${esc(game.away_team)} ${num(game.away_expected_points)} — ${num(game.home_expected_points)} ${esc(game.home_team)}</p>${scoreRange}<p><b>Confidence:</b> ${esc(game.confidence_rating)} · <span class="muted">${esc(game.data_quality_status)}</span></p><p class="muted"><b>Weather:</b> ${esc(game.weather_summary || 'Forecast unavailable')} · Context only</p>${warning}</div><div id="td-${index}" class="tab-panel">${playerRows(game.players,'td')}</div><div id="rush-${index}" class="tab-panel">${playerRows(game.players,'rush')}</div><div id="receive-${index}" class="tab-panel">${playerRows(game.players,'receive')}</div><div id="value-${index}" class="tab-panel"><p class="muted">Downstream comparison. Market prices did not create this prediction.</p>${valueCompact(valueRows)}</div><div id="insight-${index}" class="tab-panel"><h3>Model leans ${esc(winner)}</h3><ul>${factors.map(x => `<li>${esc(x)}</li>`).join('')}</ul>${warning}</div></div></details></article>`;
}
...
async function gamesPage() {
  try {
    ...
    const current = await loadJSON('data/current_week.json');
    const currentWeek = Number(current.metadata?.week);
    const data = requested && requested !== currentWeek ? await loadJSON(`data/week_${String(requested).padStart(2,'0')}.json`) : current;
    const value = Number(data.metadata?.week) === currentWeek ? await loadOptionalJSON('data/value.json', {values:[],metadata:{status:'MARKET_DATA_UNAVAILABLE'}}) : {values:[],market_game_context:{},metadata:{status:'HISTORICAL_WEEK'}};
    ...
    const available = current.metadata?.available_weeks || [currentWeek];
    const shownWeek = Number(meta.week);
    const previous = Math.max(...available.filter(week => week < shownWeek), -1);
    const next = Math.min(...available.filter(week => week > shownWeek), Infinity);
    $('#previous-week').disabled = previous < 0;
    $('#next-week').disabled = !Number.isFinite(next);
    $('#previous-week').onclick = () => { location.search = `?week=${previous}`; };
    $('#next-week').onclick = () => { location.search = next === currentWeek ? '' : `?week=${next}`; };
    $('#week-nav-label').textContent = shownWeek === currentWeek ? `Week ${shownWeek} · Current` : `Week ${shownWeek} · Completed`;
